...
class RequestsViewModel @Inject constructor(val mainRepository: MainRepository): ViewModel(){
    ...
    private val _searchedRequests = MutableLiveData<Result<QuerySnapshot>>()
    val searchedRequests: LiveData<Result<QuerySnapshot>> = _searchedRequests
    ...
    }fun fetchSearchedDocuments(searchedText: String){
        mainRepository.fetchSearchedDocuments(Constants.FOOD_REQUESTS, currentUserId, searchedText)
            .addOnSuccessListener {querySnapshot->
                if(querySnapshot.isEmpty){
                    _messageFromViewModel.value = "No Food item matched your search: $searchedText"
                }else{
                    _searchedRequests.value = Result.success(querySnapshot)
                }
            }
            .addOnFailureListener {
                _messageFromViewModel.value = it.message
